...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (action === 'toggle-availability') {
      ...
      return { statusCode: 200, headers: CORS, body: JSON.stringify({ ok: true, activo: activo !== false }) };
    }
    ...
    if (action === 'reject-tramite') {
      ...
      if (newArq) {
        ...
        return { statusCode: 200, headers: CORS, body: JSON.stringify({ ok: true, reassigned: true, new_arq: `${newArq.nombre} ${newArq.apellido}` }) };
        ...
        return { statusCode: 200, headers: CORS, body: JSON.stringify({ ok: true, reassigned: false }) };
      }
    }
