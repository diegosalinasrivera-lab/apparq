...
async function verifyProject(numUpper, emailLower) {
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!project_number) {
      return { statusCode: 400, headers: CORS, body: JSON.stringify({ error: 'Se requiere número de proyecto' }) };
    }
    ...
    if (action === 'get-updates') {
      ...
      return { statusCode: 200, headers: CORS, body: JSON.stringify({ updates }) };
    }

    /* Acciones que requieren verificación email */
    if (!email) {
      return { statusCode: 400, headers: CORS, body: JSON.stringify({ error: 'Se requiere email' }) };
    }
    const emailLower = email.trim().toLowerCase();
    const project    = await verifyProject(numUpper, emailLower);

    if (!project) {
      return { statusCode: 404, headers: CORS, body: JSON.stringify({ error: 'No se encontró el proyecto. Verifica el número y el email.' }) };
    }
    ...
    if (action === 'send-note') {
      ...
      if (!nota) {
        return { statusCode: 400, headers: CORS, body: JSON.stringify({ error: 'La consulta no puede estar vacía' }) };
      }
